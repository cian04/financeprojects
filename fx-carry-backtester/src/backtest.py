"""
backtest.py — Vectorised FX carry + momentum backtest engine.

Usage:
    python src/backtest.py

Outputs saved to results/:
    cumulative_returns.png, drawdown.png, annual_returns.png,
...
from signals import combined_signal, rank_signal
...
def run_backtest() -> dict:
    ...
    raw_signal     = combined_signal(carry_diff, log_ret, MOMENTUM_DAYS)[pairs]
...
def plot_cumulative(net: pd.Series, gross: pd.Series) -> None:
    ...
    ax.axvspan(pd.Timestamp("2020-01-01"), pd.Timestamp("2020-06-01"), alpha=0.08, color=RED,    label="COVID crash")
    ax.axvspan(pd.Timestamp("2022-01-01"), pd.Timestamp("2022-12-31"), alpha=0.08, color=ORANGE, label="2022 rate shock")
    ax.set_title("FX Carry + Momentum — Cumulative returns (2015–2024)", fontweight="bold")
...
def main() -> None:
    ...
    perf = summary(net, "FX Carry + Momentum", RF_ANNUAL)
    print("\n" + "="*45)
